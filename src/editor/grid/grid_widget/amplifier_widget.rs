...
impl AmplifierWidget {
    fn amplifier_ui(&mut self, ui: &mut egui::Ui, amp_bridge: &mut AmplifierUiBridge) {
        ...
        let volume = self.volume_smoother.tick(amp_bridge.get_out_volume());
        ...
        ctx.bridge
            .with_module_bridge(module_id, |_bridge, module_bridge| {
                if let ModuleBridge::Amplifier(amp_bridge) = module_bridge {
                    self.amplifier_ui(ui, amp_bridge);
                }
            });
    }
}
